...
jb_DOMString ProcessingInstruction_target(const ProcessingInstruction *self) {
    return em_Val_as(jb_DOMString, em_Val_get(CharacterData_as_val(self->inner), "target"));
}


CSSStyleSheet ProcessingInstruction_sheet(const ProcessingInstruction *self) {
    return em_Val_as(CSSStyleSheet, em_Val_get(CharacterData_as_val(self->inner), "sheet"));
}
