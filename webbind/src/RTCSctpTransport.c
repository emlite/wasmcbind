...
RTCDtlsTransport RTCSctpTransport_transport(const RTCSctpTransport *self) {
    return em_Val_as(RTCDtlsTransport, em_Val_get(EventTarget_as_val(self->inner), em_Val_from("transport")));
}


RTCSctpTransportState RTCSctpTransport_state(const RTCSctpTransport *self) {
    return em_Val_as(RTCSctpTransportState, em_Val_get(EventTarget_as_val(self->inner), em_Val_from("state")));
}


double RTCSctpTransport_maxMessageSize(const RTCSctpTransport *self) {
    return em_Val_as(double, em_Val_get(EventTarget_as_val(self->inner), em_Val_from("maxMessageSize")));
}


unsigned short RTCSctpTransport_maxChannels(const RTCSctpTransport *self) {
    return em_Val_as(unsigned short, em_Val_get(EventTarget_as_val(self->inner), em_Val_from("maxChannels")));
}
