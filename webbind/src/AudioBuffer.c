...
AudioBuffer AudioBuffer_new(const jb_Any* options) : em_Val(em_Val_global("AudioBuffer").new_(em_Val_from(options))) {
        return AudioBuffer(em_Val_new(em_Val_global("AudioBuffer", em_Val_from(options)));
      }
