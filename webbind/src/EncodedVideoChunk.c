...
EncodedVideoChunk EncodedVideoChunk_new(const jb_Any* init) : em_Val(em_Val_global("EncodedVideoChunk").new_(em_Val_from(init))) {
        return EncodedVideoChunk(em_Val_new(em_Val_global("EncodedVideoChunk", em_Val_from(init)));
      }


EncodedVideoChunkType EncodedVideoChunk_type(const EncodedVideoChunk *self) {
    return em_Val_as(EncodedVideoChunkType, em_Val_get(em_Val_as_val(self->inner), "type"));
}
