...
jb_Float32Array XRLightEstimate_sphericalHarmonicsCoefficients(const XRLightEstimate *self) {
    return em_Val_as(jb_Float32Array, em_Val_get(em_Val_as_val(self->inner), "sphericalHarmonicsCoefficients"));
}


DOMPointReadOnly XRLightEstimate_primaryLightDirection(const XRLightEstimate *self) {
    return em_Val_as(DOMPointReadOnly, em_Val_get(em_Val_as_val(self->inner), "primaryLightDirection"));
}


DOMPointReadOnly XRLightEstimate_primaryLightIntensity(const XRLightEstimate *self) {
    return em_Val_as(DOMPointReadOnly, em_Val_get(em_Val_as_val(self->inner), "primaryLightIntensity"));
}
