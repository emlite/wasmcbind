...
OrientationType ScreenOrientation_type(const ScreenOrientation *self) {
    return em_Val_as(OrientationType, em_Val_get(EventTarget_as_val(self->inner), "type"));
}


unsigned short ScreenOrientation_angle(const ScreenOrientation *self) {
    return em_Val_as(unsigned short, em_Val_get(EventTarget_as_val(self->inner), "angle"));
}
