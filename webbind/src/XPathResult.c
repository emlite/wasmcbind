...
unsigned short XPathResult_resultType(const XPathResult *self) {
    return em_Val_as(unsigned short, em_Val_get(em_Val_as_val(self->inner), "resultType"));
}


double XPathResult_numberValue(const XPathResult *self) {
    return em_Val_as(double, em_Val_get(em_Val_as_val(self->inner), "numberValue"));
}


jb_DOMString XPathResult_stringValue(const XPathResult *self) {
    return em_Val_as(jb_DOMString, em_Val_get(em_Val_as_val(self->inner), "stringValue"));
}


bool XPathResult_booleanValue(const XPathResult *self) {
    return em_Val_as(bool, em_Val_get(em_Val_as_val(self->inner), "booleanValue"));
}


Node XPathResult_singleNodeValue(const XPathResult *self) {
    return em_Val_as(Node, em_Val_get(em_Val_as_val(self->inner), "singleNodeValue"));
}


bool XPathResult_invalidIteratorState(const XPathResult *self) {
    return em_Val_as(bool, em_Val_get(em_Val_as_val(self->inner), "invalidIteratorState"));
}


unsigned long XPathResult_snapshotLength(const XPathResult *self) {
    return em_Val_as(unsigned long, em_Val_get(em_Val_as_val(self->inner), "snapshotLength"));
}
