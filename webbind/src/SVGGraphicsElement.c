...
bool SVGBoundingBoxOptions_fill(const SVGBoundingBoxOptions *self) {
    return em_Val_as(bool, em_Val_get(em_Val_as_val(self->inner), "fill"));
}
...
SVGAnimatedTransformList SVGGraphicsElement_transform(const SVGGraphicsElement *self) {
    return em_Val_as(SVGAnimatedTransformList, em_Val_get(SVGElement_as_val(self->inner), "transform"));
}
