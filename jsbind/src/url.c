...
bool jb_URLSearchParams_get(
    ...
    em_Val v = em_Val_call(
        params->inner, "get", em_Val_from_string(key)
    );
    if (em_Val_is_null(v)) {
        return false;
    }
...
void jb_URLSearchParams_append(
    ...
    em_Val_call(
        params->inner,
        "append",
        em_Val_from_string(key),
        em_Val_from_string(value)
    );
}
...
jb_URL jb_URL_new(const char *input) {
    return (jb_URL
          ...
          em_Val_global("URL"), em_Val_from_string(input)
      )};
}
...
#define URL_GETTER(name, type)                             \
    jb_##type jb_URL_##name(const jb_URL *url) {           \
        return (jb_##type                                  \
              ...
              em_Val_get(url->inner, em_Val_from(#name))}; \
    }
...
void jb_URL_set_search(
    jb_URL *url, const jb_String *value
) {
    em_Val_set(
        url->inner, em_Val_from("search"), value->inner
    );
}
...
jb_String jb_URL_to_json(const jb_URL *url) {
    return (jb_String
    ){.inner = em_Val_call(url->inner, "toJSON")};
}
