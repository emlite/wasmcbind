...
jb_Function jb_Function_new(
    ...
    emlite_val_push(
        arr, em_Val_as_handle(em_Val_from_string(body))
    );
    em_Val ret =
        em_Val_from_handle(emlite_val_construct_new(
            em_Val_as_handle(ctor), arr
        ));
...
Handle trampoline(Handle args, Handle data) {
    ...
    uintptr_t fn = em_Val_as(
        uintptr_t, em_Val_get(obj, em_Val_from("fn"))
    );
    jb_Any d         = (jb_Any
    ){.inner = em_Val_get(obj, em_Val_from("data"))};
...
jb_Function jb_Function_from(
    ...
    em_Val_set(
        obj, em_Val_from("fn"), em_Val_from((uintptr_t)cb)
    );
    em_Val_set(obj, em_Val_from("data"), data->inner);
    em_Val val =
        em_Val_make_fn(trampoline, em_Val_as_handle(obj));
