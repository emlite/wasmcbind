...
bool jb_Set_delete(jb_Set *set, const jb_Any *value) {
    return em_Val_as_bool(
        em_Val_call(set->inner, "delete", value->inner)
    );
}

void jb_Set_clear(jb_Set *set) {
    em_Val_call(set->inner, "clear");
}
...
jb_WeakSet jb_WeakSet_new() {
    return (jb_WeakSet
    ){.inner = em_Val_new(em_Val_global("WeakSet"))};
}
